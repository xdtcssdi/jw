<template>
  <div class="main-login">
    ...
    <el-dialog :title="'学生信息'" :visible.sync="messageVisible" width="33%" center
               class="user-dialog">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click=updateStudent v-if="isModify">更 新</el-button>
        <el-button type="primary" @click=saveStudent v-else>添 加</el-button>
...
export default {
  ...
  methods: {
    queryList() {
      ...
      this.$http.get('http://localhost:8080/student/', {params: params}).then(({body}) => {
...
// 上传文件之前的钩子, 参数为上传的文件,若返回 false 或者返回 Promise 且被 reject，则停止上传
    ...
    uploadFile(file) {
      ...
      this.$http.post('http://localhost:8080/user/upload_student', form, config).then(function (res) {
        if (res.status === 200) {
          if (res.body) {
    ...
    modifyStudent(id) {
      this.isModify = true;
      this.$http.get('http://localhost:8080/student/' + id).then(({body}) => {
        if (body.success === true) {
          let params = {
            'id': id,
